...
AI_BSDF_EXPORT_METHODS(A2BsdfMicrofacetConductorMtd);
namespace a2 {
...
create_microfacet_conductor(AtShaderGlobals* sg, AtRGB weight, AtVector N,
                            ...
                            float rx, float ry) {
    auto bsdf = AiBSDF(sg, weight, A2BsdfMicrofacetConductorMtd,
                       sizeof(BsdfMicrofacetConductor));
    auto bsdf_mf =
        reinterpret_cast<a2::BsdfMicrofacetConductor*>(AiBSDFGetData(bsdf));
    // Don't forget to call new() on the memory or the vtable won't be set up...
    new (bsdf_mf) BsdfMicrofacetConductor;
    bsdf_mf->N = N;
    bsdf_mf->U = U;
    bsdf_mf->_fresnel = FresnelConductor{reflectivity, edgetint};
