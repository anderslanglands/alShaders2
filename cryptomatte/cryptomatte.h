...
inline void hash_name_rgb(const char* name, AtRGB* out_color) {
...
public:
    CryptomatteData() {
        ...
        this->set_option_channels(CRYPTO_DEPTH_DEFAULT,
                                  CRYPTO_PREVIEWINEXR_DEFAULT);
        this->set_option_namespace_stripping(CRYPTO_STRIPOBJNS_DEFAULT,
                                             CRYPTO_STRIPMATNS_DEFAULT);
        this->set_option_ice_pcloud_verbosity(CRYPTO_ICEPCLOUDVERB_DEFAULT);
    }

    void setup_all(const AtString aov_cryptoasset,
                   const AtString aov_cryptoobject,
                   const AtString aov_cryptomaterial, AtArray* uc_aov_array,
    ...
    void set_option_ice_pcloud_verbosity(int verbosity) {
        ...
        g_pointcloud_instance_verbosity =
            this->option_pcloud_ice_verbosity; // to do: really remove this
    }
...
private:
    void do_standard_cryptomattes(AtShaderGlobals* sg) {
        if (!this->aov_array_cryptoasset && !this->aov_array_cryptoobject &&
            !this->aov_array_cryptomaterial)
            return;
        ...
        if (this->aov_array_cryptoasset)
            write_array_of_AOVs(sg, this->aov_array_cryptoasset,
                                nsp_hash_clr.r);
        if (this->aov_array_cryptoobject)
            write_array_of_AOVs(sg, this->aov_array_cryptoobject,
                                obj_hash_clr.r);
        if (this->aov_array_cryptomaterial)
            write_array_of_AOVs(sg, this->aov_array_cryptomaterial,
                                mat_hash_clr.r);
        ...
        if (CRYPTOMATTE_CACHE[sg->tid].object == sg->Op) {
            ...
            *obj_hash_clr = CRYPTOMATTE_CACHE[sg->tid].obj_hash_clr;
            ...
            bool cachable = get_object_names(
                sg, sg->Op, this->option_strip_obj_ns, nsp_name, obj_name);
        ...
        if (CRYPTOMATTE_CACHE[sg->tid].shader_object == sg->Op) {
            *mat_hash_clr = CRYPTOMATTE_CACHE[sg->tid].mat_hash_clr;
            ...
            cachable = get_material_name(sg, sg->Op, shader,
                                         this->option_strip_mat_ns, mat_name) &&
                       cachable;
    ...
    void setup_cryptomatte_nodes() {
        ...
        for (uint32_t i = 0; i < prev_output_num; i++) {
            ...
            if (strcmp(aov_name, this->aov_cryptoasset.c_str()) == 0) {
                this->aov_array_cryptoasset =
                    AiArrayAllocate(this->option_aov_depth, 1, AI_TYPE_STRING);
            ...
            if (cryptoAOVs != NULL) {
                ...
                this->create_AOV_array(aov_name, filter_name, camera_name,
                                       driver, cryptoAOVs, &new_outputs);
            }
        }
        ...
        this->build_standard_metadata(driver_cryptoAsset_v,
                                      driver_cryptoObject_v,
                                      driver_cryptoMaterial_v);
    ...
    void write_standard_sidecar_manifests() {
        ...
        this->compile_standard_manifests(do_md_asset, do_md_object,
        ...
        if (do_md_object)
            write_manifest_sidecar_file(map_md_object,
                                        this->manif_object_paths);
        if (do_md_material)
            write_manifest_sidecar_file(map_md_material,
                                        this->manif_material_paths);

        // reset sidecar writers
        this->manif_asset_paths = StringVector();
        this->manif_object_paths = StringVector();
        this->manif_material_paths = StringVector();
    }
    ...
    void compile_standard_manifests(bool do_md_asset, bool do_md_object,
        ...
        while (!AiNodeIteratorFinished(shape_iterator)) {
            ...
            get_object_names(NULL, node, this->option_strip_obj_ns, nsp_name,
                             obj_name);
            ...
            if (do_md_material) {
                ...
                for (uint32_t i = 0; i < AiArrayGetNumElements(shaders); i++) {
                    ...
                    if (!shader)
                        continue;
                    get_material_name(NULL, node, shader,
                                      this->option_strip_mat_ns, mat_name);
    ...
    void write_user_sidecar_manifests() {
        ...
        for (size_t i = 0; i < this->user_cryptomattes.count; i++) {
            ...
            for (size_t j = 0; j < this->manifs_user_paths[i].size(); j++)
                do_metadata[i] = do_metadata[i] &&
                                 this->manifs_user_paths[i][j].length() > 0;
        }
        this->compile_user_manifests(do_metadata, manf_maps);

        for (size_t i = 0; i < this->manifs_user_paths.size(); i++)
            if (do_metadata[i])
                write_manifest_sidecar_file(manf_maps[i],
                                            this->manifs_user_paths[i]);

        this->manifs_user_paths = std::vector<StringVector>();
    }
            ...
            for (uint32_t i = 0; i < this->user_cryptomattes.count; i++) {
                if (do_metadata[i])
                    add_override_udata_to_manifest(
                        node, this->user_cryptomattes.sources[i], manf_maps[i]);
            }
        }
        AiNodeIteratorDestroy(shape_iterator);
    }
    ...
    void build_standard_metadata(std::vector<AtNode*> driver_asset_v,
        ...
        for (size_t i = 0; i < driver_object_v.size(); i++) {
            if (metadata_needed(driver_object_v[i], this->aov_cryptoobject)) {
                do_md_object = true;
                metadata_set_unneeded(driver_object_v[i],
                                      this->aov_cryptoobject);
                break;
            }
        }
        for (size_t i = 0; i < driver_material_v.size(); i++) {
            if (metadata_needed(driver_material_v[i],
                                this->aov_cryptomaterial)) {
                do_md_material = true;
                metadata_set_unneeded(driver_material_v[i],
                                      this->aov_cryptomaterial);
                break;
            }
        }
        ...
        for (size_t i = 0; i < driver_asset_v.size(); i++) {
            this->setup_deferred_manifest(
                driver_asset_v[i], this->aov_cryptoasset,
                this->manif_asset_paths[i], manif_asset_m);
            write_metadata_to_driver(driver_asset_v[i], this->aov_cryptoasset,
                                     map_md_asset, manif_asset_m);
        }
        this->manif_object_paths.resize(driver_object_v.size());
        for (size_t i = 0; i < driver_object_v.size(); i++) {
            this->setup_deferred_manifest(
                driver_object_v[i], this->aov_cryptoobject,
                this->manif_object_paths[i], manif_object_m);
            write_metadata_to_driver(driver_object_v[i], this->aov_cryptoobject,
                                     map_md_object, manif_object_m);
        }
        this->manif_material_paths.resize(driver_material_v.size());
        for (size_t i = 0; i < driver_material_v.size(); i++) {
            this->setup_deferred_manifest(
                driver_material_v[i], this->aov_cryptomaterial,
                this->manif_material_paths[i], manif_material_m);
            write_metadata_to_driver(driver_material_v[i],
                                     this->aov_cryptomaterial, map_md_material,
                                     manif_material_m);
        }
        ...
        for (uint32_t i = 0; i < drivers_vv.size(); i++) {
            ...
            for (size_t j = 0; j < drivers_vv[i].size(); j++) {
                ...
                if (sidecar) {
                    ...
                    this->setup_deferred_manifest(
                        driver, user_aov, manif_asset_paths, manif_user_m);
                    this->manifs_user_paths[i].push_back(
